  ...
  public:
    ...
    { 
      ...
      simu_timer_ = this->create_wall_timer(
        ...
        std::bind(&MujocoGym::simu_step, this));
  ...
  private:
    ...
    void keyboard_imp(int key, int act){
      ...
      if (act==GLFW_PRESS && key==KEYCODE) {
        ...
        mj_resetDataKeyframe(m, d, 0);
        lock.unlock();
        RCLCPP_INFO(this->get_logger(), "Simulation has been restarted");
      }
    }
    ...
    mjtNum* jacp = new mjtNum[18];      // pointer to position jacobian of tip, 3x6
    mjtNum* jacr = new mjtNum[18];      // pointer to rotation jacobian of tip, 3x6
